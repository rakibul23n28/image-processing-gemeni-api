...
#define I2S_DOUT      5    // DIN
#define I2S_BCLK      18   // BCLK
#define I2S_LRC       4    // LRC (WS)
...
void setupI2SOutput() {
  i2s_config_t i2s_config_out = {
    ...
    .channel_format = I2S_CHANNEL_FMT_RIGHT_LEFT, // stereo output typical for DAC
...
void sendToServer(camera_fb_t* fb) {
  ...
  delay(100); // Give server time to start sending

    // --- Play WAV audio response directly ---
  uint8_t buffer[512];
  size_t bytes_written;
  unsigned long lastDataTime = millis();
  size_t totalResponseSize = 0;  // To track total bytes received

  unsigned long timeout = millis();
  while (millis() - timeout < 10000) { // up to 10s wait
    while (client.available()) {
      int len = client.read(buffer, sizeof(buffer));
      if (len > 0) {
        i2s_write(I2S_NUM_1, buffer, len, &bytes_written, portMAX_DELAY);
        totalResponseSize += len;
        timeout = millis(); // reset timeout after activity
      }
    }
  ...
  client.stop();
  Serial.printf("Audio response complete. Total response size: %u bytes\n", totalResponseSize);
...
void setup() {
  ...
  connectWiFi();
  setupI2SMic();    // for I2S_NUM_0 input mic
  setupI2SOutput();
  Serial.println("Setup complete.");
}
void loop() {
  static unsigned long lastAutoTrigger = 0;
  const unsigned long autoTriggerInterval = 60000; // 1 min

  unsigned long now = millis();

  // Debug print every 5 seconds to confirm loop is running
